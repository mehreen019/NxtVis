...
def extract_cv_info_with_ollama(cv_text: str) -> Dict[str, str]:
    ...
    def call_with_retries():
        ...
        for i in range(max_retries):
            try:
                response = client.generate(model=ollamamodel, prompt=message)
...
def main(cv_folder: Path, job_folder: Path, output_folder: Path):
    ...
    for job_name, top_match in top_matches.items():
        ...
        try:
            info = extract_cv_info_with_ollama(cv_text)
        except Exception as e:
            print(f"Error extracting CV info for '{cv_name}': {e}")
            continue
...
if __name__ == "__main__":
    ...
    main(cv_folder, job_folder, output_folder)